...
var MenuTree = function MenuTree(container, params) {
  ...
  s.initData = function (list, ulContainer) {
    for (var i = 0, option; option = list[i++];) {
      ...
      var html = '<div data-index="' + i + '" ' + s.params.itemAttr + '=\'' + (0, _stringify2.default)(option) + '\' class="' + s.params.tagClass + (option.id === s.params.selectedId ? ' active' : '') + '" id="ID-MenuTree' + option.id + '">' + '<p class="menutree-tag-font">' + option.name + '</p>' + (option.children && option.children.length > 0 ? '<i class="menutree-more"></i>' : '') + '</div>';
      li.innerHTML = html;
