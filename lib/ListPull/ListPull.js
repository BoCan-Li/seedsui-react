  ...
  (0, _createClass3.default)(ListPull, [{
    ...
    value: function render() {
      ...
      return _react2.default.createElement(
        ...
        { ref: function ref(el) {
            _this3.$el = el;
          }, className: 'listpull' + (className ? ' ' + className : ''), style: style },
        list.map(function (item, index) {
          return _react2.default.createElement(
            'li',
            { key: 'button' + index, 'data-index': '' + index, className: 'border-b listpull-li' },
            item.lButtons && item.lButtons.length && _react2.default.createElement(
              'div',
              { className: 'listpull-left' },
              item.lButtons.map(function (button, i) {
                return _react2.default.createElement(
                  _Button2.default,
                  { key: 'button' + i, 'data-index': '' + index, 'data-i': '' + i, 'data-direction': 'left', className: 'listpull-button' + (button.className ? ' ' + button.className : ''), style: button.style },
                  button.value
                );
              })
            ),
            _react2.default.createElement(
              'div',
              { className: 'listpull-handler', 'data-index': '' + index },
              item.container
            ),
            item.rButtons && item.rButtons.length && _react2.default.createElement(
              'div',
              { className: 'listpull-right' },
              item.rButtons.map(function (button, i) {
                return _react2.default.createElement(
                  _Button2.default,
                  { key: 'button' + i, 'data-index': '' + index, 'data-i': '' + i, 'data-direction': 'right', className: 'listpull-button' + (button.className ? ' ' + button.className : ''), style: button.style },
                  button.value
                );
              })
            )
