...
var ImgLazy = function ImgLazy(params) {
  ...
  s.updateCompleteList = function (src) {
    s.completeList = _DB2.default.getSession('seedsui_imglazy_complete_list') || [];
    if (src) {
      s.completeList.push(src);
      _DB2.default.setSession('seedsui_imglazy_complete_list', s.completeList);
    }
  };
  s.updateCompleteList();
  // 是否已加载
  s.isComplete = function (src) {
    if (s.completeList.indexOf(src) !== -1) {
      return true;
    }
    return false;
  };
  ...
  s.load = function () {
    ...
    // 获取所有需要加载的图片
    s.imgs = [].slice.call(s.overflowContainer.querySelectorAll('[' + s.params.loadAttr + ']')).filter(function (n) {
