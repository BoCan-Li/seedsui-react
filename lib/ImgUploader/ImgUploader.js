  ...
  function ImgUploader(props) {
    ...
    _this.filterUploadFail = function (imgMap) {
      ...
      if (failIndexs && failIndexs.length) {
        _Bridge2.default.showToast('' + (window._seeds_lang['the'] || '第') + failIndexs.join(',') + (window._seeds_lang['th_image_failed_to_upload'] || '张图片上传失败') + ', ' + (window._seeds_lang['re-photo_upload'] || '请重新拍照上传'), { mask: false });
      }
      return list;
    };
...
ImgUploader.propTypes = {
  ...
  chooseRepeat: _propTypes2.default.bool, // 是否允许重复选择照片
