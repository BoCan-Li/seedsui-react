    ...
    _this.correctDate = function (value) {
      ...
      if (min && (/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(min) || /^[0-9]{2}:[0-9]{2}$/.test(min))) {
        if (type === 'date' && selectDate.compareDate(min.toDate()) === -1) {
          if (onError) {
            onError((0, _extends3.default)({ msg: (window._seeds_lang['hint_cannot_be_less_than'] || '不能小于') + min, select: text, min: min, current: current }, error));
            return false;
          }
          text = min;
      ...
      if (max && (/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(max) || /^[0-9]{2}:[0-9]{2}$/.test(max))) {
        if (type === 'date' && selectDate.compareDate(max.toDate()) === 1) {
          if (onError) {
            onError((0, _extends3.default)({ msg: (window._seeds_lang['hint_cannot_be_greater_than'] || '不能大于') + max, select: text, max: max, current: current }, error));
            return false;
          }
          text = max;
  ...
  (0, _createClass3.default)(InputDate, [{
    ...
    value: function render() {
      var _props = this.props,
          ...
          others = (0, _objectWithoutProperties3.default)(_props, ['valueForKey', 'split', 'min', 'max', 'type', 'onClick', 'onChange', 'onError', 'pickerStyle', 'pickerClassName', 'pickerMaskStyle', 'pickerMaskClassName', 'pickerShow', 'onClickSubmit', 'onClickCancel', 'onClickMask']);
