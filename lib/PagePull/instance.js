...
var PagePull = function PagePull(container, params) {
  ...
  s.onTouchMove = function (e) {
    ...
    // 左滑
    if (s.touches.diffX + s.touches.posX < -s.rightClientWidth) s.touches.diffX = -s.rightClientWidth - s.touches.posX;
    // 右滑
    if (s.touches.diffX + s.touches.posX > s.leftClientWidth) s.touches.diffX = s.leftClientWidth - s.touches.posX;

    // 滑动
    s.showMask();
    e.currentTarget.style.webkitTransform = 'translateX(' + (s.touches.diffX + s.touches.posX) + 'px)';
  };
