...
var Picker = function Picker(params) {
  ...
  s.addSlot = function (values, defaultKey, classes) {
    ...
    if (classes.indexOf(s.params.lockClass) >= 0) slot.isLock = true;else slot.isLock = false;
  ...
  s.replaceSlot = function (index, values, defaultKey, classes, fn) {
    ...
    if (classes.indexOf(s.params.lockClass) >= 0) s.slotbox.children[index].isLock = true;else s.slotbox.children[index].isLock = false;
    // 渲染
    s.renderSlot(slot);
    // 回调
    if (fn) fn(s);
  ...
  s.renderSlot = function (slot) {
    ...
    for (var i = 0; i < values.length; i++) {
      // 获得defaultIndex
      if (slot.defaultKey && slot.defaultKey == values[i][s.params.dataKeyPropertyName]) {
        // eslint-disable-line
        defaultIndex = i;
      }
  ...
  s.onClickMask = function (s) {
    if (s.params.onClickMask) s.params.onClickMask(s);else s.hide();
  };

  // Submit|Cancel
  s.onClickSubmit = function (s) {
    if (s.params.onClickSubmit) s.params.onClickSubmit(s);else s.hide();
  };
  s.onClickCancel = function (s) {
    if (s.params.onClickCancel) s.params.onClickCancel(s);else s.hide();
  };
