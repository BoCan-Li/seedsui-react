...
// 用于身份证信息查询,很大,如不需要,不要加载
// 用于视频音频
// 用于判断浏览器是否处于全屏状态,如不需要，不要加载
// 用于富文本,如不需要，不要加载
// 用于路由监听,如不需要，不要加载
// 用于帧率测试一次动画等,如不需要，不要加载
...
exports.Bridge = _Bridge2.default; // 用于表单校验,如不需要,不要加载

// bridge
// 用于订阅发布模式
// 用于路由监听,如不需要，不要加载
// 用于表单序列化等操作,如不需要，不要加载
