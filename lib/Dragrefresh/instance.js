...
// Dragrefresh 下拉刷新
var Dragrefresh = function Dragrefresh(params) {
  /* ----------------------
  Model
  ---------------------- */
  var defaults = {
  ...
  s.setPagination = function (isNext, isNoData, isError) {
    // 下一页 | 无数据 | 错误
    s.isNoData = isNoData;
    // 如果加载错误
    if (isError) {
  ...
  s.onTouchStart = function (e) {
    ...
    s.preventMove = true;
    // 如果不在顶部，则不触发
    if (s.getScrollTop() <= s.params.isTopPosition) s.touches.isTop = true;else s.touches.isTop = false;
