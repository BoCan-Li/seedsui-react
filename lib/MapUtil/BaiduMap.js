...
var BaiduMap = function BaiduMap(id, params) {
  ...
  s.formatPoints = function (points) {
    ...
    if ((0, _stringify2.default)(points).indexOf('lng') !== -1) return points;
  ...
  s.pointsToPolygons = function (argPointss) {
    var polygons = [];
    for (var i = 0; i < argPointss.length; i++) {
      var polygon = s.pointsToPolygon(argPointss[i]);
  ...
  s.addContextMenu = function (overlay) {
    ...
    try {
      ...
      for (var _iterator3 = (0, _getIterator3.default)(options.menus.entries()), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        _loop();
      }
    } catch (err) {
