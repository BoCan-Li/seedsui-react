...
(function () {
        ...
        for (var i = 0; i < oldpoints.length; i++) {
            if (WqGeoUtils.isPointInPolygon(oldpoints[i], newpoly)) {
                ...
                for (var t = 0; t < tnewpoints.length; t++) {
                    ...
                    if (crossPoint) {
                        //找到第一个交点，判断那个点在之外
                        if (WqGeoUtils.isPointInPolygon(tnewpoints[t], oldpoly)) {
                            //第一个点在多边形内
                            if (WqGeoUtils.isPointInPolygon(tnewpoints[t + 1], oldpoly)) {//第二个点也在多边形内，不可能
                                ...
                                for (var findex = t + 1; findex < tnewpoints.length - 1; findex++) {
                                    if (!out && WqGeoUtils.isPointInPolygon(tnewpoints[findex], oldpoly)) {} else {
                                        out = true;
                            ...
                            if (WqGeoUtils.isPointInPolygon(tnewpoints[t + 1], oldpoly)) {
                                //第一个点在多边形外，第二个在多边形内
                                ...
                                var out = false;
                                for (var findex = t + 1; findex < tnewpoints.length - 1; findex++) {
                                    if (!out && WqGeoUtils.isPointInPolygon(tnewpoints[findex], oldpoly, false)) {} else {
                                        out = true;
                                        arr.push(tnewpoints[findex]);
                                    }
                                }
     ...
     */
    WqGeoUtils.segmentsIntr = function (a, b, c, d) {
        /** 1 解线性方程组, 求线段交点. **/
        // 如果分母为0 则平行或共线, 不相交
