...
var Calendar = function Calendar(container, params) {
  ...
  s.draw = function (vertical) {
    ...
    for (var i = 0; i < s.dates.length; i++) {
      ...
      // class-currentClass
      if (s.data[i].isCurrent) s.dates[i].classList.add(s.params.currentClass);else s.dates[i].classList.add(s.params.notcurrentClass);
      // class-todayClass
