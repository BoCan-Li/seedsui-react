...
// Tree 树结构
var Tree = function Tree(container, params) {
  ...
  s.initData = function (id, ulContainer) {
    ...
    for (var i = 0, option; option = group[i++];) {
      ...
      var copyOption = (0, _create2.default)(option);
      // line的data-xxx属性html
      var lineDataHTML = '';
      for (var n in option) {
      ...
      copyOption.html = '<div class="' + s.params.iconClass + '">' + '<i class="' + s.params.arrowClass + '"></i>' + '</div>' + '<div class="' + s.params.titleClass + '">' + option.name + '</div>';
      // Callback onData
  ...
  s.isSelected = function (id, parentid) {
    ...
    for (var i = 0, opt; opt = parentNodes[i++];) {
      // eslint-disable-line
  ...
  s.onClickTree = function (e) {
    ...
    if (s.target.classList.contains(s.params.lineClass)) {
      // 点击二级
      s.targetLine = s.target;
      s.targetLi = s.target.parentNode;
    } else if (s.target.classList.contains(s.params.iconClass) || s.target.classList.contains(s.params.titleClass)) {
      // 点击三级
      s.targetLine = s.target.parentNode;
      s.targetLi = s.target.parentNode.parentNode;
    }
    if (s.target.classList.contains(s.params.buttonAddClass)) {
      // 点击添加
      s.onClickBtnAdd(e);
    } else if (s.target.classList.contains(s.params.buttonDelClass)) {
      // 点击删除
      s.onClickBtnDel(e);
