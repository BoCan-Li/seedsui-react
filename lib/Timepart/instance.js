...
// Timepart 时间段
var Timepart = function Timepart(container, params) {
  /* -----------------------
  Model
  ----------------------- */
  var defaults = {
  ...
  s.createParts = function () {
    ...
    for (var i = 0; i < s.rowsCount; i++) {
      ...
      for (var j = 0; j < s.params.colCount; j++) {
        ...
        part.innerHTML = '<span class=' + s.params.partStartClass + '>' + startHour + ':' + startMinute + '</span>' + '<span class=' + s.params.partEndClass + '>' + endHour + ':' + endMinute + '</span>';
        s.parts.push(part);
  ...
  s.onClickContainer = function (e) {
    if (e.target.classList.contains(s.params.partClass) || e.target.classList.contains(s.params.progressClass)) {
      // 点击part
      s.onClickPart(e);
    }
  };
