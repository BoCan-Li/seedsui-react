...
var Timepart = function (_Component) {
  ...
  function Timepart(props) {
    ...
    _this.update = function () {
      ...
      for (var i = 0, time; time = _this.props.times[i++];) {
        ...
        if (time.className && time.startTime && time.endTime) {
          _this.state.instance.addProgress(time.startTime, time.endTime, time.className, time.data || null, time.cover || false);
        }
      }
      // onChange
      _this.onChange();
    ...
    _this.onChange = function () {
      var times = _this.state.instance.getTimes();
      var part = _this.state.instance.container.querySelector('.timepart-part.active');
      if (part) {
        times.push({
