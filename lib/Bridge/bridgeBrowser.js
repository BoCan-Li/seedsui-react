...
var Bridge = {
  ...
  */
  ...
  uploadImage: function uploadImage() {
    ...
    setTimeout(function () {
      _this.hideLoading();
      _this.showToast(window._seeds_lang['uploaded_completed'] || '上传完成', { mask: false });
      var res = {
  ...
  */
  Image: function Image() {
    ...
    s.choose = function () {
      ...
      if (!Bridge.debug) {
        if (params.onChooseFail) params.onChooseFail({ errMsg: 'chooseImage:' + (window._seeds_lang['hint_only_app_and_wx'] || '此功能仅可在微信或APP中使用') });else Bridge.showToast(window._seeds_lang['hint_only_app_and_wx'] || '此功能仅可在微信或APP中使用', { mask: false });
        return;
      }
      var option = {
