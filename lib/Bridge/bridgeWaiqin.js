  ...
  videoRecord: function videoRecord() {
    ...
    wq.wqjnc.videoRecord(function (res) {
      ...
      if (res.result === '1') {
        ...
        if (params.fail) params.fail({ errMsg: 'videoRecord:录制失败' });else _bridgeBrowser2.default.showToast('录制失败', { mask: false });
      }
  ...
  videoUpload: function videoUpload() {
    ...
    wq.wqjnc.videoUpload(function (res) {
      ...
      if (res.result === '1') {
        ...
        if (params.fail) params.fail({ errMsg: 'videoUpload:上传失败' });else _bridgeBrowser2.default.showToast('上传失败', { mask: false });
      }
    }, (0, _stringify2.default)(params));
  },
  /* -----------------------------------------------------
  ...
  scanQRCode: function scanQRCode() {
    ...
    wq.wqhardware.getQrCode(function (res) {
      ...
      if (res && res.qrCode) {
        ...
        if (params.fail) params.fail({ errMsg: 'scanQRCode:扫码失败请稍后重试' });else _bridgeBrowser2.default.showToast('扫码失败请稍后重试', { mask: false });
      }
    });
  },
  /**
  ...
  getLocation: function getLocation() {
    ...
    wq.wqlocation.getLocationBackground(function (res) {
      ...
      if (res && res.wqLatitude) {
        ...
        if (params.fail) params.fail({ errMsg: 'getLocation:定位失败,请检查定位权限是否开启' });else _bridgeBrowser2.default.showToast('定位失败,请检查定位权限是否开启', { mask: false });
      }
  ...
  getLocationMap: function getLocationMap() {
    ...
    wq.wqlocation.getLocationMap(function (res) {
      ...
      if (res && res.wqLatitude) {
        ...
        if (params.fail) params.fail({ errMsg: 'getLocationMap:定位失败,请检查外勤365定位权限是否开启' });else _bridgeBrowser2.default.showToast('定位失败,请检查外勤365定位权限是否开启', { mask: false });
      }
