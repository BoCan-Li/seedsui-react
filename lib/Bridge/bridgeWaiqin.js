...
var Bridge = {
  ...
  */
  ...
  qmfpay: function qmfpay(params, callback) {
    wq.wqpay.qmfpay(function (result) {
      // eslint-disable-line
      if (callback) callback(result);
    }, null, params ? (0, _stringify2.default)(params) : null);
  },
  ...
  scanQRCode: function scanQRCode() {
    ...
    wq.wqhardware.getQrCode(function (res) {
      ...
      if (res && res.qrCode) {
        ...
        if (params.fail) params.fail({ errMsg: 'scanQRCode:' + (window._seeds_lang['hint_scan_failed'] || '扫码失败') + ', ' + (window._seeds_lang['hint_try_again_later'] || '请稍后重试') });else Bridge.showToast('scanQRCode:' + (window._seeds_lang['hint_scan_failed'] || '扫码失败') + ', ' + (window._seeds_lang['hint_try_again_later'] || '请稍后重试'), { mask: false });
      }
    });
  },
  /**
