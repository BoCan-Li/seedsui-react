  ...
  scanQRCode: function scanQRCode() {
    ...
    wx.scanQRCode({ // eslint-disable-line
      ...
      fail: function fail(res) {
        if (params.onError) params.onError({ code: 'qrcodeFail', msg: '扫码失败,请退出重试' + res, res: res });else _bridgeBrowser2.default.showToast('扫码失败,请退出重试', { mask: false });
      },
  ...
  Image: function Image(params) {
    ...
    s.choose = function () {
      ...
      Bridge.chooseImage((0, _assign2.default)({
        ...
        fail: function fail(res) {
          if (params.onChooseError) params.onChooseError({ code: 'chooseFail', msg: '选择照片失败' + res, res: res });else _bridgeBrowser2.default.showToast('选择照片失败', { mask: false });
        },
