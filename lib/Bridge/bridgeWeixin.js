  ...
  config: function config() {
    ...
    _ApiAxios2.default.get(ticketUrl, opts.params || ticketParams).then(function (response) {
      ...
      if (result.code === '1') {
        ...
        if (!wx) {
          // eslint-disable-line
          if (opts.onError) opts.onError({ code: 'bridgeInitFail', msg: '微信组件下载失败,如需使用本地能力,请返回重试' });else _bridgeBrowser2.default.showToast('微信组件下载失败,如需使用本地能力,请返回重试', { mask: false });
          return;
        }
        // 记录日志
        ...
        wx.error(function (res) {
          ...
          if (opts.onError) opts.onError({ code: 'oauthFail', msg: errMsg });else _bridgeBrowser2.default.showToast(errMsg, { mask: false });
        });
      } else {
      ...
      if (opts.onError) opts.onError({ code: 'oauthInterfaceFail', msg: 'getJsApiTicket微信鉴权失败,请稍后重试' });else _bridgeBrowser2.default.showToast('getJsApiTicket微信鉴权失败,请稍后重试', { mask: false });
    });
  },
  // 关闭窗口
