...
var Bridge = {
    ...
    wx.getLocation({ // eslint-disable-line
      ...
      success: function success(res) {
        ...
        if (res.latitude && res.latitude) {
          ...
          if (params.fail) params.fail(res);else _bridgeBrowser2.default.showToast('没有获取到经纬度', { mask: false });
        }
      },
      fail: function fail(res) {
        _this.locating = false;
        if (params.fail) params.fail(res);else _bridgeBrowser2.default.showToast('定位失败,请检查微信定位权限是否开启', { mask: false });
      },
  ...
  scanQRCode: function scanQRCode() {
    ...
    wx.scanQRCode({ // eslint-disable-line
      ...
      fail: function fail(res) {
        if (params.fail) params.fail(res);else _bridgeBrowser2.default.showToast('扫码失败,请退出重试', { mask: false });
      },
  ...
  Image: function Image(params) {
    ...
    s.choose = function () {
      ...
      Bridge.chooseImage((0, _assign2.default)({
        ...
        fail: function fail(res) {
          if (params.onChooseFail) params.onChooseFail(res);else _bridgeBrowser2.default.showToast('选择照片失败', { mask: false });
        },
