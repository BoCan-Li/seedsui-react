...
var Bridge = {
  ...
  getLocation: function getLocation() {
    ...
    this.locationOvertime = setTimeout(function () {
      _this.locationOvertime = null;
      if (!_DB2.default.getCookie('app_location')) {
        var errMsg = '请确认微信定位权限是否开启,如未开启将影响图片上传功能';
        if (params.onError) params.onError({ code: 'locationFail', msg: errMsg, res: {} });else _bridgeBrowser2.default.showToast(errMsg, { mask: false });
      }
    }, params.timeout || 5000);
    // 调用定位
    ...
    wx.getLocation({ // eslint-disable-line
      ...
      success: function success(res) {
        ...
        if (res.latitude && res.latitude) {
          ...
          if (params.onError) params.onError({ code: 'locationFail', msg: errMsg, res: res });else _bridgeBrowser2.default.showToast(errMsg, { mask: false });
        }
      },
  ...
  scanQRCode: function scanQRCode() {
    ...
    wx.scanQRCode({ // eslint-disable-line
      ...
      fail: function fail(res) {
        if (params.onError) params.onError({ code: 'qrcodeFail', msg: '扫码失败,请退出重试' + (0, _stringify2.default)(res), res: res });else _bridgeBrowser2.default.showToast('扫码失败,请退出重试', { mask: false });
      },
  ...
  Image: function Image(params) {
    ...
    s.choose = function () {
      ...
      Bridge.chooseImage((0, _assign2.default)({
        ...
        fail: function fail(res) {
          if (params.onChooseError) params.onChooseError({ code: 'chooseFail', msg: '选择照片失败' + (0, _stringify2.default)(res), res: res });else _bridgeBrowser2.default.showToast('选择照片失败', { mask: false });
        },
