...
var Bridge = {
  ...
  config: function config() {
    ...
    _ApiAxios2.default.get(serviceUrl, opts.params || ticketParams).then(function (response) {
      var result = response;
      if (result.code === '1') {
        if (opts.onSuccess) {
          opts.onSuccess({ result: result, status: '1' });
        } else {
          _this.wxSignature(opts, result);
        }
      } else {
        if (opts.onError) opts.onError({ code: 'oauthInterfaceFail', msg: response.message });else _bridgeBrowser2.default.showToast(response.message, { mask: false });
      }
    }).catch(function (err) {
      errMsg = '微信鉴权接口异常,请稍后重试';
      if (opts.onError) opts.onError({ code: 'oauthInterfaceFail', msg: errMsg });else _bridgeBrowser2.default.showToast(errMsg, { mask: false });
    });
  },
  wxSignature: function wxSignature(opts, result) {
    var errMsg = '';
    var params = {
    ...
    if (!wx) {
      ...
      if (opts.onFail) opts.onFail({ code: 'bridgeInitFail', msg: errMsg });else _bridgeBrowser2.default.showToast(errMsg, { mask: false });
      return;
    }
    ...
    wx.error(function (res) {
      ...
      _DB2.default.setSession('bridge_isready', '-1');
      // Callback
      errMsg = '微信签名过期导致验证失败' + (0, _stringify2.default)(res);
      if (opts.onFail) opts.onFail({ code: 'oauthFail', msg: errMsg });else _bridgeBrowser2.default.showToast(errMsg, { mask: false });
    });
  },
  // 关闭窗口
