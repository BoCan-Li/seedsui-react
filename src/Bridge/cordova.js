...
var Bridge = {
  ...
  */
  ...
  videoRecord: function (params = {}) {
    var self = this
    if (Device.compareVersion(Device.platformVersion, '6.2.2') < 0) {
      self.showToast(getLocaleValue('hint_video_record_version') || '视频录制功能需要升级至6.2.2及以上的客户端', {mask: false})
      return
    }
    wq.wqjnc.videoRecord((res) => { // eslint-disable-line
      if (res.result === '1'){
        ...
        else self.showToast(getLocaleValue('hint_video_record_version') || '录制失败', {mask: false})
      }
  ...
  videoUpload: function (params = {}) {
    var self = this
    if (Device.compareVersion(Device.platformVersion, '6.2.2') < 0) {
      self.showToast(getLocaleValue('hint_video_upload_version') || '视频上传功能需要升级至6.2.2及以上的客户端', {mask: false})
      return
    }
    wq.wqjnc.videoUpload((res) => { // eslint-disable-line
      if (res.result === '1'){
        ...
        else self.showToast(getLocaleValue('hint_video_upload_failed') || '上传失败', {mask: false})
      }
    }, JSON.stringify(params))
  },
  /* -----------------------------------------------------
  ...
  videoInfo: function (params = {}) {
    var self = this
    if (Device.compareVersion(Device.platformVersion, '6.2.2') < 0) {
      self.showToast(getLocaleValue('hint_video_info_version') || '视频功能需要升级至6.2.2及以上的客户端', {mask: false})
      return
    }
    wq.wqjnc.videoInfo((res) => { // eslint-disable-line
      if (res.result === '1') {
        ...
        if (params.fail) params.fail({errMsg: `videoInfo:${getLocaleValue('hint_video_info_failed') || '未查到此视频信息'}`})
      }
    }, JSON.stringify(params))
  },
  /* -----------------------------------------------------
  ...
  scanQRCode: function (params = {}) {
    ...
    wq.wqhardware.getQrCode((res) => { // eslint-disable-line
      if (res && res.qrCode) {
        ...
        else self.showToast(`scanQRCode:${getLocaleValue('hint_scan_failed') || '扫码失败'}, ${getLocaleValue('hint_try_again_later') || '请稍后重试'}`, {mask: false})
      }
    })
  },
  /**
  ...
  getLocation: function (params = {}) {
    ...
    wq.wqlocation.getLocationBackground((res) => { // eslint-disable-line
      ...
      if (res && res.wqLatitude) {
        ...
        if (params.fail) params.fail({errMsg: `getLocation: ${getLocaleValue('hint_location_failed') || '定位失败,请检查定位权限是否开启'}`})
        else self.showToast(getLocaleValue('hint_location_failed') || '定位失败, 请检查定位权限是否开启', {mask: false})
      }
  ...
  getLocationMap: function (params = {}) {
    ...
    wq.wqlocation.getLocationMap((res) => { // eslint-disable-line
      if (res && res.wqLatitude) {
        ...
        if (params.fail) params.fail({errMsg: `getLocationMap:${getLocaleValue('hint_location_map_failed') || '定位失败, 请检查外勤365定位权限是否开启'}`})
        else self.showToast(getLocaleValue('hint_location_map_failed') || '定位失败, 请检查外勤365定位权限是否开启', {mask: false})
      }
  ...
  getCustomerAreaMore: function (params = {}) { // {selectedIds: 'id,id', success([{id: '', name: ''}])}
    var self = this
    if (Device.compareVersion(Device.platformVersion, '6.2.2') < 0) {
      self.showToast(getLocaleValue('hint_get_customer_area_more_version') || '此功能需要升级至6.2.2及以上的客户端', {mask: false})
      return
    }
  ...
  openNativePage: function (params = {ios: {}, android: {}}) {
    var self = this
    if (!params.ios.url) {
      self.showToast(getLocaleValue('hint_open_native_page_must_ios_url') || 'ios参数url不能为空', {mask: false})
      return
    }
    if (!params.android.url) {
      self.showToast(getLocaleValue('hint_open_native_page_must_android_url') || 'android参数url不能为空', {mask: false})
      return
    }
