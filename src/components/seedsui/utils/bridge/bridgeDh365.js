...
const registerEvents = ['getGoodsByApp', 'getCartGoodsByApp', 'onBackPress', 'setOnlineByApp'];
...
export default new class WqJsBridge {
  ...
  * */
  isHomePage = (callback) => {
    this.invoke('isHomePage', null, callback);
  }
  /*
  * 商联支付
  * params: {appKey:'', dealerCode:'', orderId:'', payAmount:''}
  * */
  slopenpay = (params, callback) => {
  ...
  */
  ...
  /** 去首页 */
  goHome = (callback) => this.invoke('goHome', null, callback);

  /** 返回绑定 **/
  back = () => {
    const isFromApp = Request.getParameter('isFromApp') ? Request.getParameter('isFromApp') : '0';
    if (isFromApp === '1') {
      try {
        this.closeWindow();
      } catch (error) {
      ...
      try {
        // this.showTab();
        this.goHome((result) => {
          if (result.from === 'goods') {
            this.props.pushState('/_react_/goods?isFromApp=1');
            return;
          }
        });
