var CanvasUtil = {
  cropImg: function (opts = {}) {
    ...
    var errMsg = ''
    if (isNaN(opts.x)) {
      errMsg = 'SeedsUI Error:CanvasUtil类cropImg裁切方法x参数不正确'
      console.warn(errMsg)
      return errMsg
    }
    if (isNaN(opts.y)) {
      errMsg = 'SeedsUI Error:CanvasUtil类cropImg裁切方法y参数不正确'
      console.warn(errMsg)
      return errMsg
    }
    if (isNaN(opts.width)) {
      errMsg = 'SeedsUI Error:CanvasUtil类cropImg裁切方法width参数不正确'
      console.warn(errMsg)
      return errMsg
    }
