...
const Preview = forwardRef(({
  ...
  return <Swipe ref={refEl} containerChildren={children} defaultIndex={activeIndex} onHide={hideHandler} onChange={changeHandler} {...others}>
    {list.map((source, index) => {
      return <div className="swiper-slide" key={index}>
        <div className="swiper-zoom-container">
          {source.type !== 'video' && <img className="swiper-zoom-target" src={source.src}/>}
          {source.type === 'video' &&
            <VideoFull
              ...
              autoPlay={false}
              bar={
                <div className="videofull-close" onClick={onHide}></div>
              }
            />
          }
          {source.children}
